...
// Get the function calling tool generated by the openAI UI. No need to do it programatically
...
// This is the function that gets called from openAI's function calling
...
eventHandler.on('event', async (event: AssistantStreamEvent) => {
  try {
    ...
    if (
      ...
      const { client_email, prompt } =
        api_thread_res.metadata as IAI_Quote_Create['metadata'];
      const message = data.content[0].text.value;

      // Update our DB for a new quote
      const quoteModel = QuotesModel({
        client_email,
        prompt,
        message
      });
      await Quotes_upsert(quoteModel);

      // save memory. Ideally this would be a delete on redis but this can suffice
...
// Entry point for when a client wants to message openAI. This should also be stored as an action
// but this has been implemented as a Route to show how further code organization would look like
